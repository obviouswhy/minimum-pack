const { Builder, By, Key, WebDriver, Alert } = require("selenium-webdriver");
...
it("HOMEページタイトルテスト", async () => {
    ...
    //ページに移動
    await driver.get("https://minimumskills.jp");
    //タイトル取得
...
it("Contactフォームテスト", async () => {
    ...
    const driver = await new Builder().forBrowser("chrome").build();
    //ページに移動
    await driver.get("https://minimumskills.jp/contact.html");
    //タイトルsyつ億
    const title = await driver.getTitle();
    //タイトル評価
